...
import { ShoppingCart, Scale, BadgeCent, Check } from "lucide-react";
...
export default function ProductCard({ product }) {
  ...
  const image = product?.images || product?.image || "/images/fallback-product.png";
  ...
  return (
    <Card className="group overflow-hidden rounded-2xl border border-gray-200 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-border dark:bg-card">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative mx-auto h-[400px] w-[350px] overflow-hidden rounded-t-2xl bg-gradient-to-b from-[#fbf7ef] to-[#f4efe6] dark:from-muted/40 dark:to-muted/10">
          <Badge
          ...
          <img
            src={image}
            alt={product.name}
            className="h-full w-full object-contain pt-5 transition duration-500 group-hover:scale-105"
          />
        </div>
      </Link>
      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={!isInStock}
            className="TheamColor ml-auto inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-black shadow-sm transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
          >
